...
# The second column, you reason, must be what you should play in response: X for Rock, Y for Paper, and Z for Scissors. Winning every 
# time would be suspicious, so the responses must have been carefully chosen.
...
# In the first round, your opponent will choose Rock (A), and you should choose Paper (Y). This ends in a win for you with a score of 
...
with open('day2input.txt') as inputlines:
    [print(line.strip()) for line in input.readlines()]
